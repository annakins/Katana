...
float XLoc = PolarToRectangularX(TeleportTarget.GetPositionX(), fdir, -300)
float YLoc = PolarToRectangularY(TeleportTarget.GetPositionY(), fdir, -300)	;to do: add prop instead of fixed numbers
...
;float FRand = utility.RandomFloat()
	if (self.GetDistance(Player) > 2000)&&(FollowerRecruited.GetValue() ==1)&&(Megara.GetActorValue("WaitingForPlayer") == 0)
		;if (FRand > 0.2) 
		bCasting = True
